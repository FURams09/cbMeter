...
function buildMapForStation() {
    ...
    $('select option').each(function (i, option) {
        ...
        marker.addListener('mouseover', function () { setScreen($option.val()); $('#station').selectmenu('refresh');  $('#meter').progressbar({ value: getStationFullPct($option.data().bikes, $option.data().racks) });$('#meter').show(500); })
        marker.addListener('mouseout', function () { $('#meter').hide(500); })
    });
    
    setScreen($('select option:selected').val(), true);
